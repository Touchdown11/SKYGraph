...
If they are missing you can generate them manually once before the first run:
...
**Automatic training:** the main entry points already handle this for you —
`runSkyGraphDashboard()`, `runStage14Visualization()`, and
`runStage15Experiments()` each call `setupSkyGraph()` internally, so they will
**automatically train the Stage 12 GAT weights and the Stage 13 PPO agent** if
they are not present. Running `setupSkyGraph()` yourself is only needed if you
want to train on your own schedule or see the progress interactively.
...
- Trained artifacts: **auto-generated by the entry points** (see section 4) —
  if `gatThreatWeightsStage12.mat` / `ppoAgentStage13.mat` are missing they are
  trained automatically on the first run.
...
Self-contained: it auto-trains any missing artifacts, configures the pipeline,
...
Also auto-trains missing artifacts, then simulates Stage 13 and plays the 3-D
mission scene with a fixed camera.
