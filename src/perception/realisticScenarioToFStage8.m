function [distanceM,valid,status,directionDistance,directionValid, ...
    ...
    idealDistance,faultFlags,hitEntityId,hitEntityType] = ...
    realisticScenarioToFStage8(xTrue,t,entityPositionWorld,entityRadius, ...
    entityId,entityType,entityActive,groundZ,ceilingZ,ambientMode,faultMode)
%REALISTICSCENARIOTOFSTAGE8 Stage 7 faults applied to Stage 8 entities.
%
...
persistent initialized randomState generatedSequence totalDropCount ...
    pendingDistance pendingValid pendingStatus pendingIdeal pendingHitId pendingHitType ...
    pendingSequence pendingTimestamp pendingFaultFlags pendingAvailable ...
    receiverDistance receiverValid receiverStatus receiverIdeal receiverHitId receiverHitType ...
...
if isempty(initialized)
    ...
    pendingHitId = zeros(10,1);
    pendingHitType = zeros(10,1);
    pendingSequence = 0.0;
    ...
    receiverHitId = zeros(10,1);
    receiverHitType = zeros(10,1);
...
if pendingAvailable
    ...
    receiverHitId = pendingHitId;
    receiverHitType = pendingHitType;
...
[idealCurrent,idealValid,idealStatus,idealHitId,idealHitType] = ...
...
if packetWasDropped
    ...
    pendingHitId = idealHitId;
    pendingHitType = idealHitType;
...
hitEntityId = receiverHitId;
hitEntityType = receiverHitType;
dropCount = totalDropCount;
if receiverSequence <= 0.0
    % No packet has arrived yet. Distances/valid flags are already invalid,
    % so use age zero instead of an artificial 1e6-second startup spike.
    packetAge = 0.0;
else
